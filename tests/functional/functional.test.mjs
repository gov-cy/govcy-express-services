...
async function runPa11yOnCurrentUrl({ browser, context, sourcePage, pa11yOptions }) {
  const pa11yPage = await context.newPage();

  const cookies = await sourcePage.cookies();
  if (cookies?.length) await pa11yPage.setCookie(...cookies);

  const url = sourcePage.url();
  await pa11yPage.goto(url, { waitUntil: 'networkidle0' });
...
describe('Functional Test - Login and Navigate', function () {
  ...
  it('2. should change the language to English when clicking the EN link', async () => {
    ...
    pa11yResults = await runPa11yOnCurrentUrl({
      browser,
      context,
      sourcePage: page,
      pa11yOptions: {
  ...
  it('3. should submit the form and display an error summary', async () => {
    ...
    pa11yResults = await runPa11yOnCurrentUrl({
      browser,
      context,   // <- keep a reference (see below)
      sourcePage: page,
      pa11yOptions: {
        standard: pa11yStandard,
        ignore: pa11yIgnoreErrors
